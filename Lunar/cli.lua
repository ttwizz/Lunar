...
local function lines_from(file)
    ...
    local lines = {}
	lines[#lines + 1] = [[getfenv().LUNAR_WHITELIST=function(a)assert(a,"GroupId missing or invalid")local b=game:GetService("GroupService")local c=0;local d=pcall(function()local a=game:GetService("MarketplaceService"):GetProductInfo(game.PlaceId)if game.CreatorType==Enum.CreatorType.Group then _,c=pcall(function()return b:GetGroupInfoAsync(a.Creator.CreatorTargetId).Owner.Id end)elseif game.CreatorType==Enum.CreatorType.User then c=game.CreatorId end end)if not d then return false end;local b=b:GetGroupsAsync(c)for b,b in next,b do if b.Id==a then return true end end;return false end]]
